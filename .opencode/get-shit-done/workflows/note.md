...
- **Global scope**: `D:/projetos/opencodeDeepseek/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/projetos/opencodeDeepseek/.opencode/notes/`)
...
2. Glob `D:/projetos/opencodeDeepseek/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (D:/projetos/opencodeDeepseek/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/projetos/opencodeDeepseek/.opencode/notes/` — works in any directory
