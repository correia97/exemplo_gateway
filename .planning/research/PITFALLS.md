...
**Why it happens:** The `Microsoft.OpenApi` package auto-generates server URLs based on the request that hit `MapScalarApiReference()`. When accessed through APISIX, the `Host` header is different from the original request.
