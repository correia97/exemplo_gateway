...
export async function getCharacters(filters: CharacterFilters = {}): Promise<PaginatedResponse<Character>> {
  ...
  return apiFetch<PaginatedResponse<Character>>(`${DRAGONBALL_API_URL}/api/v1/characters${qs ? `?${qs}` : ''}`)
}

export async function getCharacter(id: number): Promise<Character> {
  return apiFetch<Character>(`${DRAGONBALL_API_URL}/api/v1/characters/${id}`)
}

export async function createCharacter(data: CharacterCreatePayload): Promise<Character> {
  return apiFetch<Character>(`${DRAGONBALL_API_URL}/api/v1/characters`, {
    method: 'POST',
...
export async function deleteCharacter(id: number): Promise<void> {
  return apiFetch<void>(`${DRAGONBALL_API_URL}/api/v1/characters/${id}`, { method: 'DELETE' })
}