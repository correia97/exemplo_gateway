...
- **Global scope**: `D:/projetos/opencodeDeepseek/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/projetos/opencodeDeepseek/.claude/notes/`)
...
2. Glob `D:/projetos/opencodeDeepseek/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/projetos/opencodeDeepseek/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/projetos/opencodeDeepseek/.claude/notes/` — works in any directory
